  ...
  return (
    <div className="min-h-screen page-bg">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6">
          <SearchInput value={searchQuery} onChange={handleSearch} placeholder="Search pending approvals by customer name, IMEI, or Aadhaar..." loading={loading} />
        </div>
        ...
        {requests === null && (
          <div className="flex flex-col items-center justify-center py-20 text-center animate-fade-in">
            ...
            <p className="text-ink-muted text-sm mt-1">Privacy rule: no data shown until searched</p>
          </div>
        )}

        {requests !== null && requests.length === 0 && (
          <div className="text-center py-16">
            <p className="text-success font-medium">All caught up! No pending requests.</p>
          </div>
        )}
              ...
              return (
                <div key={req.id} className="card p-5 animate-fade-in">
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
                    ...
                    <Link href={`/receipt/${req.id}`} target="_blank" className="text-xs text-info hover:text-info underline underline-offset-4">
                      View Receipt →
                    </Link>
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                    ...
                    {req.fine_amount > 0 && (
                      <div className="bg-surface-2 rounded-xl p-3">
                        <p className="text-xs text-danger mb-1">Fine</p>
                        <p className="font-num font-semibold text-danger">{fmt(req.fine_amount)}</p>
                      </div>
                    )}
                    {req.first_emi_charge_amount > 0 && (
                      <div className="bg-surface-2 rounded-xl p-3">
                        <p className="text-xs text-brand-600 mb-1">1st EMI Charge</p>
                        <p className="font-num font-semibold text-brand-600">{fmt(req.first_emi_charge_amount)}</p>
                      </div>
                    )}
                  ...
                  <div className="flex items-center gap-2 text-xs text-ink-muted mb-4">
                    <span className={`font-semibold ${req.mode === 'UPI' ? 'text-info' : 'text-success'}`}>{req.mode}</span>
      ...
      {rejectModal && (
        <div className="modal-backdrop">
          <div className="card w-full max-w-md p-6 animate-slide-up">
            <h3 className="font-display text-xl font-bold text-danger mb-2">Reject Payment Request</h3>
            <p className="text-sm text-ink-muted mb-4">The EMIs will be reverted to UNPAID. Provide a reason.</p>
            <div className="mb-4">
              <label className="form-label">Rejection Reason <span className="text-brand-600">*</span></label>
              <textarea
                value={rejectReason}
